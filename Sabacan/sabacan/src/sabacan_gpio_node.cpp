...
public:
  SabaneCanNode() : Node("sabacan_gpio_node")
  {
    ...
    this->declare_parameter(
      "pin_type", std::vector<std::string>{
                    "PWM_OUT", "PWM_OUT", "PWM_OUT", "PWM_OUT", "PWM_OUT", "PWM_OUT", "PWM_OUT",
                    "PWM_IN", "ESC"});
    ...
    sabacan_ref_subscription_ = this->create_subscription<sabacan_msgs::msg::SabacanGPIORef>(
      "/sabacan_gpio_ref" + std::to_string(board_id), 100,
    ...
    RCLCPP_INFO(
      this->get_logger(),
      "GPIO node initialized with monitor period: %d ms, enable_monitor_period: %s, "
      "gpio_monitor_timer: %d ms",
...
private:
    ...
    for (int i = 0; i < N; ++i) {
      if (pin_type[i] == "PWM_IN" && pwm_data_updated_[i]) {
        ...
        if (period_cnt > 0) {
          ...
          duty_ratio =
            std::min(1.0f, static_cast<float>(duty_cnt) / static_cast<float>(period_cnt));
        }
      ...
      for (int i = 0; i < N; ++i) {
        ...
        if (new_level != prev_level) {
          ...
          } else if (prev_level && !new_level)  // Falling edge
          {
            ...
            if (
              last_rise_time_[i].nanoseconds() > 0 &&
  ...
  void gpio_init()
  {
    ...
    while (retry_count < max_init_retries_) {
      try {
        RCLCPP_INFO(
          this->get_logger(), "Starting GPIO initialization (attempt %d/%d)...", retry_count + 1,
          max_init_retries_);
        ...
        RCLCPP_INFO(
          this->get_logger(),
          "GPIO init parameters: monitor_period_ms=%d, enable_monitor_period=%s", monitor_period_ms,
          enable_monitor_period ? "true" : "false");
        ...
        RCLCPP_INFO(
          this->get_logger(), "Setting GPIO configuration: PORT_MODE=0x%04X, PORT_INT_EN=0x%04X",
        ...
        RCLCPP_INFO(
          this->get_logger(), "Setting monitor register mask: 0x%016llX",
          static_cast<unsigned long long>(monitor_reg_mask));

        // 定期フィードバックを有効化（パラメータで設定）
        RCLCPP_INFO(
          this->get_logger(), "Calling setMonitorReg(0x%016llX)...",
          static_cast<unsigned long long>(monitor_reg_mask));
        ...
        RCLCPP_INFO(
          this->get_logger(),
          "GPIO init completed successfully: PORT_MODE=0x%04X, PORT_INT_EN=0x%04X, "
          "MONITOR_REG=0x%016llX, MONITOR_PERIOD=%dms, ENABLE_MONITOR=%s",
          port_mode_mask, port_int_en_mask, static_cast<unsigned long long>(monitor_reg_mask),
          monitor_period_ms, enable_monitor_period ? "true" : "false");

        // 成功時もループを継続（パラメータで設定した回数まで）
        retry_count++;
        RCLCPP_INFO(
          this->get_logger(), "GPIO initialization attempt %d/%d completed successfully",
          retry_count, max_init_retries_);

        // 最後の試行でない場合は待機してから次の試行
        if (retry_count < max_init_retries_) {
          RCLCPP_INFO(
            this->get_logger(), "Waiting %d ms before next attempt...", init_retry_delay_ms_);
        ...
        retry_count++;
        RCLCPP_WARN(
          this->get_logger(), "GPIO initialization attempt %d/%d failed: %s", retry_count,
          max_init_retries_, e.what());

        if (retry_count >= max_init_retries_) {
          RCLCPP_ERROR(
            this->get_logger(), "GPIO initialization failed after %d attempts", max_init_retries_);
          throw std::runtime_error(
            "GPIO initialization failed after maximum retries: " + std::string(e.what()));
        }
    ...
    RCLCPP_INFO(
      this->get_logger(), "GPIO initialization completed: %d attempts executed", max_init_retries_);
  }
  ...
  std::string pin_type[9] = {"PWM_OUT", "PWM_OUT", "PWM_OUT", "PWM_OUT", "PWM_OUT",
                             "PWM_OUT", "PWM_OUT", "PWM_IN",  "ESC"};
