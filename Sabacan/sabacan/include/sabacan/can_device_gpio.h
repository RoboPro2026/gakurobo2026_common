...
public:
  ...
  void setPortMode(uint16_t val) { tx(GPIO::PORT_MODE, val); }

  void setPortWrite(uint16_t val) { tx(GPIO::PORT_WRITE, val); }

  void setPortIntEn(uint16_t val) { tx(GPIO::PORT_INT_EN, val); }

  void setEscModeEn(uint16_t val) { tx(GPIO::ESC_MODE_EN, val); }

  void setPwmPeriod(int n, uint16_t val) { tx(GPIO::PWM_PERIOD + n, val); }

  void setPwmDuty(int n, uint16_t val) { tx(GPIO::PWM_DUTY + n, val); }

  void setMonitorPeriod(uint16_t val) { tx(GPIO::MONITOR_PERIOD, val); }

  void setMonitorReg(uint64_t val) { tx(GPIO::MONITOR_REG, val); }
