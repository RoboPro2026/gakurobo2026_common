...
> **補足**  
> 起動直後に大量のパラメータをCAN経由で書き込むと基板側が取りこぼすことがあったため、`sabacan_robomasv2_node` は各パラメータ送信のあとに約10msのディレイを入れています。起動に数百ミリ秒余分にかかりますが、通信を安定させるための仕様です。また、delayの間は処理が止まるので注意が必要です。
